...
bool MakeMove(std::vector<std::vector<char>>& board,
              ...
              int col,
              char player) {
  if (row < 0 || row > kBoardSize - 1 || col < 0 || col > kBoardSize - 1) { // Wrong bounds, 0 didn't work as input, 3 worked but shouldn't
...
char CheckWinner(const std::vector<std::vector<char>>& board) {
  ...
  if (board[0][0] == board[1][1] && board[1][1] == board[2][2] && // Opposite diagonal was not tested, just noticed while checking code
      board[0][0] != ' ') {
    return board[0][0];
  }
  return ' ';
